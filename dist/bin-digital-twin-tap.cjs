...
function getUserId(opts = {}) {
  try {
    ...
    const email = (0, import_node_child_process.execSync)("git config user.email", execOpts).trim();
...
function getMachineId(machineIdFile = DEFAULT_PATHS.machineIdFile) {
  ...
  const id = `${(0, import_node_os2.hostname)()}-${ulid().slice(-8).toLowerCase()}`;
...
function claudeTranscriptPath(home, cwd, sessionId) {
  return (0, import_node_path7.join)(home, ".claude", "projects", projectDirForCwd(cwd), `${sessionId}.jsonl`);
}
function tapSession(input, deps = {}) {
  try {
    const home = (deps.homedir ?? import_node_os4.homedir)();
    const ulidFn = deps.ulid ?? ulid;
    const now = deps.now ?? (() => /* @__PURE__ */ new Date());
    ...
    let sourceSize = 0;
    try {
      sourceSize = (0, import_node_fs5.statSync)(transcriptPath).size;
    } catch {
    }
    ...
    const paths = digitalTwinPaths(home);
    (0, import_node_fs5.mkdirSync)(paths.pendingDir, { recursive: true });
    const id = ulidFn();
    const payloadPath = (0, import_node_path7.join)(paths.pendingDir, `${id}.payload`);
    const metadataPath = (0, import_node_path7.join)(paths.pendingDir, `${id}.json`);
    (0, import_node_fs5.copyFileSync)(transcriptPath, payloadPath);
    let payloadSize = 0;
    try {
      payloadSize = (0, import_node_fs5.statSync)(payloadPath).size;
    } catch {
    }
    ...
    (0, import_node_fs5.writeFileSync)(metadataPath, JSON.stringify(metadata, null, 2), "utf-8");
    if (deps.daemonBin && (0, import_node_fs5.existsSync)(deps.daemonBin)) {
      const spawnFn = deps.spawn ?? import_node_child_process2.spawn;
      let logFd;
      try {
        const logPath = paths.uploaderLogFile;
        try {
          if ((0, import_node_fs5.existsSync)(logPath) && (0, import_node_fs5.statSync)(logPath).size > 1e6) {
            (0, import_node_fs5.writeFileSync)(logPath, "", "utf-8");
          }
        } catch {
        }
        logFd = (0, import_node_fs5.openSync)(logPath, "a");
      } catch {
        logFd = void 0;
      }
...
function claudeCredentialsPath(home) {
  return (0, import_node_path8.join)(home, ".claude", ".credentials.json");
}
...
function saveQuotaCache(cachePath, quota, deps = {}) {
  ...
  const wr = deps.writeFileSync ?? import_node_fs6.writeFileSync;
  try {
    md((0, import_node_path8.dirname)(cachePath), { recursive: true });
...
function loadLastHourlyScanAt(fencePath, deps = {}) {
  const ex = deps.existsSync ?? import_node_fs7.existsSync;
  const rd = deps.readFileSync ?? import_node_fs7.readFileSync;
...
function recordHourlyScanFired(fencePath, now, deps = {}) {
  const md = deps.mkdirSync ?? import_node_fs7.mkdirSync;
  const open = deps.openSync ?? import_node_fs7.openSync;
  const wrFd = deps.writeSync ?? import_node_fs7.writeSync;
  const cls = deps.closeSync ?? import_node_fs7.closeSync;
  const unl = deps.unlinkSync ?? import_node_fs7.unlinkSync;
  const wrFile = deps.writeFileSync ?? import_node_fs7.writeFileSync;
  const lockPath = `${fencePath}.lock`;
  try {
    md((0, import_node_path9.dirname)(fencePath), { recursive: true });
  } catch {
  }
  let fd;
  try {
...
function listLocalSessions(home, deps) {
  const existsSync11 = deps?.existsSync ?? import_node_fs8.existsSync;
  const readdirSync4 = deps?.readdirSync ?? import_node_fs8.readdirSync;
  const statSync6 = deps?.statSync ?? import_node_fs8.statSync;
  const projectsDir = (0, import_node_path10.join)(home, ".claude", "projects");
  if (!existsSync11(projectsDir)) {
    return [];
  }
  let projectDirs;
  try {
    projectDirs = readdirSync4(projectsDir);
  } catch {
    return [];
  }
  const results = [];
  for (const projectName of projectDirs) {
    const projectPath = (0, import_node_path10.join)(projectsDir, projectName);
    let entries;
    try {
      entries = readdirSync4(projectPath);
    } catch {
      continue;
    }
    for (const entry of entries) {
      ...
      const transcriptPath = (0, import_node_path10.join)(projectPath, entry);
      let stat;
      try {
        stat = statSync6(transcriptPath);
      } catch {
        continue;
      }
      if (!stat.isFile()) {
...
function tryWritePidLockAtomic(path3, payload) {
  try {
    (0, import_node_fs10.writeFileSync)(path3, payload, { flag: "wx", encoding: "utf-8" });
    return true;
  } catch (err) {
...
function selfDirname() {
  ...
  return import_node_path13.default.dirname((0, import_node_url.fileURLToPath)(importMetaUrl));
}
function resolveDaemonBin(home, deps = {}) {
  const ex = deps.existsSync ?? import_node_fs13.existsSync;
  const here = (deps.selfDirname ?? selfDirname)();
  const paths = digitalTwinPaths(home);
  const userInstalled = import_node_path13.default.join(paths.digitalTwinDir, "bin-uploader.cjs");
  if (ex(userInstalled)) return userInstalled;
  const sameDirBin = import_node_path13.default.join(here, "bin-uploader.cjs");
  if (ex(sameDirBin)) {
  ...
  const monorepoDist = import_node_path13.default.join(
    here,
    "..",
    "..",
    "digital-twin",
...
function selfInstallFromSource(src, dest, destDir, deps) {
  const md = deps.mkdirSync ?? import_node_fs13.mkdirSync;
  const cp = deps.copyFileSync ?? import_node_fs13.copyFileSync;
  const rn = deps.renameSync ?? import_node_fs13.renameSync;
  const ul = deps.unlinkSync ?? import_node_fs13.unlinkSync;
...
if (import_node_path13.default.basename(process.argv[1] ?? "").startsWith("bin-digital-twin-tap")) {
  main().catch(() => {
  });
}
